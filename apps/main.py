...
print("Start:---------------------->Reading the CSV Files")
symbolDF= pd.read_csv(r"/home/ElectionProject/apps/dataset/symbols.csv",encoding='utf-8-sig')
# Extract column names into a list
colNames = [x for x in symbolDF.columns]
# Create empty DataFrame with those column names
symbolDF = pd.DataFrame(columns=colNames)

wordDF= pd.read_csv(r"/home/ElectionProject/apps/dataset/words.csv",encoding='utf-8-sig')
wordListDF= pd.read_csv(r"/home/ElectionProject/apps/dataset/wordList.csv",encoding='utf-8-sig')
print("End:---------------------->Reading the CSV Files")
...
def saveKeyValue(key,values,Tokentype,matchType):
  ...
  symbolDF.loc[len(symbolDF)] = ['id_1',key, value1,value2,value3,str(Tokentype),matchType,0] 
...
def generateCV():
  print("Inside the Generate CV Method")
  tokensv = []
  tokensc = []
  sqlv = "select pattern,value2 from symbols where type = 1"
  sqlc = "select pattern,value1,value2 from symbols where type=2"
  
  vowelDF= symbolDF.loc[symbolDF['type'] == '1']
  consonentDF= symbolDF.loc[symbolDF['type'] == '2']
  
  print("<------------------>The symbolDF obtained is<-------->"+str(symbolDF))
  print("<------------------>The types in the symbol DF are<-------->"+str(symbolDF['type']))
  
  print("<----------------->The vowelDF is<------------->"+str(vowelDF))
  print("<----------------->The consonent DF is<------------->"+str(consonentDF))  
  
  # Iterate over each row 
  for index, rows in vowelDF.iterrows(): 
    # Create list for the current row 
    vowelList =[rows.pattern, rows.value2]      
    # append the list to the final list 
    tokensv.append(vowelList) 

  # Iterate over each row 
  for index, rows in consonentDF.iterrows(): 
    # Create list for the current row 
    consonentList =[rows.pattern, rows.value2]      
    # append the list to the final list 
    tokensc.append(consonentList) 
 
  ''''
  res = schemeCrusor.execute(sqlv)
  for i in res:
    print("The value obtained from the scheme is"+str(i))
    tokensv.append(i)    
...
def createSchemeFile():
  ...
  consonants({"mya" : "മ്യ"})
  consonants({'c' : "ക്"})
  print("<------------>Start: generating CV<---------------------------------->") 
  generateCV()
  print("<------------>End: generating CV<---------------------------------->")   
  consonants({
      (("ru")) : "ര്",
      (("r~", "ru")):  "റ്",
      (("nu")): "ന്",
  ...
  symbolDF.to_csv (r'/home/ElectionProject/apps/dataset/symbols.csv', index = False, header=True)
...
def isLearnedWord(word):
  ...
  res = schemeCrusor.fetchall()
  '''
  wordListFilter= wordListDF.loc[wordListDF['word'] == word] 
  print("<------------------>The wordFilter obtained is<-------->"+str(wordListFilter))
  res=[]
  # Iterate over each row 
  for index,rows in wordListFilter.iterrows(): 
    # Create list for the current row 
    wordList =[rows.word]      
    # append the list to the final list 
    res.append(wordList) 
  ...
  for i in fileList:
    ...
    for word in learningFile:
      ...
      if isLearnedWord(word):
        ...
        print("Not entering the else block")
...
createSchemeFile()
print("The current symbol DF is"+str(symbolDF))
